...
- **Retries**: single stages receive up to 3 attempts; repair/input/policy errors and cancellation are not retried. Ticket-parallel stages instead retry only failed logical tickets, up to 3 ticket attempts, from durable per-ticket results. Retry events include the failed keys and preserved blockers.
...
- **Ticket context and parallelism** (`reconcile.go`, `runTicketStage`): after architecture, the worker merges paths, dependencies, focused checks, and compact ticket constraints, then generates `artifacts/ticket-contexts.json`. Each packet contains the ticket plus only its mapped requirement/acceptance excerpts, shared non-goals/constraints, applicable ADR filenames, and pointers to full sources. The coder receives the packet as its sole generated planning input. The worker computes dependency waves, creates one detached worktree per ready ticket, and invokes a Codex coordinator with native subagents bounded by `stage.parallelism`. Valid per-ticket results are authoritative even if the aggregate coordinator summary is malformed. Successful commits are cherry-picked and checkpointed in key order; only failed tickets are retried.
