...
- **Ticket context and parallelism** (`reconcile.go`, `runTicketStage`): after architecture, the worker merges paths, dependencies, tiered verification (`iteration_checks`, `ticket_gate`, and lint/QA-owned `pipeline_gates`), and compact ticket constraints, then generates `artifacts/ticket-contexts.json`. Legacy `focused_checks` remain readable during upgrades. Each packet contains the ticket plus only its mapped requirement/acceptance excerpts, shared non-goals/constraints, applicable ADR filenames, and pointers to full sources. The coder receives that compact packet plus the current source request or operator recovery guidance; explicit recovery guidance may refine a stale restored packet for the resumed attempt. Lint and QA receive the aggregate packets so they can execute and deduplicate their owned downstream gates. The worker computes dependency waves, creates one detached worktree per ready ticket, and invokes a Codex coordinator with native subagents bounded by `stage.parallelism`. Valid per-ticket results are authoritative even if the aggregate coordinator summary is malformed. Successful commits are cherry-picked and checkpointed in key order; only failed tickets are retried.
