...
def validate_pipeline(pipeline: Any, repo: Path | None = None) -> list[str]:
    ...
    for index, stage in enumerate(stages):
        ...
        if not isinstance(result, dict):
            ...
            if not isinstance(result.get("agent"), str) or not re.fullmatch(r"[a-z][a-z0-9-]*", result["agent"]):
    ...
    if not isinstance(loops, list):
        ...
        for index, loop in enumerate(loops):
            ...
            source, target, through = loop.get("from"), loop.get("to"), loop.get("through")
            if source in loop_sources:
                errors.append(f"repair_loops[{index}].from duplicates another repair loop")
            elif source in stage_positions:
                loop_sources.add(source)
            if source in stage_positions and target in stage_positions and through in stage_positions:
                if not stage_positions[target] < stage_positions[source] <= stage_positions[through]:
                    errors.append(f"repair_loops[{index}] must re-enter an earlier stage and continue through its source stage")
    return errors
...
def validate_generic_agent_output(agent: str, data: Any) -> list[str]:
    ...
    custom = agent not in GENERIC_AGENT_CONTRACTS
    statuses, required = GENERIC_AGENT_CONTRACTS.get(
        agent,
        ({"completed", "blocked"}, {"worktree_clean", "blocker", "residual_risks"}),
    )
    errors: list[str] = []
...
def command_init_run(args: argparse.Namespace) -> None:
    ...
    if args.run_id and not re.fullmatch(r"run_[A-Za-z0-9._-]+", args.run_id):
        raise HarnessError("run-id must start with run_ and contain only letters, digits, dot, underscore, or hyphen")
    explicit_run = repo / pipeline["run_root"].replace("{run_id}", args.run_id) if args.run_id else None
    active = explicit_run if explicit_run and (explicit_run / "state.json").is_file() else (
        None if args.new_run or args.run_id else find_active_run(runs_root, args.provider, args.issue_key)
    )
    if active:
        ...
        run_id = args.run_id or f"run_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')}_{secrets.token_hex(4)}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    command.add_argument("--run-id", help="use an orchestrator-assigned run id")
