...
The versioned worker checkpoint provides Node.js 24, pnpm 11, the Playwright CLI, and system Chromium. Repositories still declare `@playwright/test` and other libraries in their package manifests and lockfiles. Cloud workers pass `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`, cap Playwright at two workers by default, and instruct agents to apply that cap explicitly; repositories should read both it and `HARNESS_PLAYWRIGHT_WORKERS` in Playwright configuration.
