...
Create the least-privilege GitHub App with `agent-harness cloud auth github --manifest-owner <organization>`. The local manifest callback requests Metadata read plus Contents, Pull Requests, and Workflows write, configures the signed `/webhooks/github` endpoint for pull-request events, then encrypts the generated private key and webhook secret without writing them to disk. Workflows write is required because repository-owned coding tickets may add or update `.github/workflows/*`; it does not authorize workflow dispatch. Install that app only on repositories the runner should operate and use the resulting installation ID during repository registration.
...
The versioned worker checkpoint provides Git, GitHub CLI, ripgrep, jq, curl, Make, Python 3 with pip and venv, Node.js 24 with npm and pnpm 11, Codex, Playwright, system Chromium, zip/unzip, file, and patch. The final template-build command executes each advertised tool (plus the Agent Harness and Railway CLIs), verifies pip and venv module support, and records resolved binary paths in `/opt/agent-harness/runtime-manifest.json`; checkpoint capture fails if the smoke test fails. Repositories still declare `@playwright/test` and other libraries in their package manifests and lockfiles. Cloud workers pass `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`, cap Playwright at two workers by default, and instruct agents to apply that cap explicitly; repositories should read both it and `HARNESS_PLAYWRIGHT_WORKERS` in Playwright configuration.

Linear parents move to Todo when claimed and In Progress when the first pipeline stage starts. Vessica acknowledges the native AgentSession immediately, publishes semantic Agent Activities for stage progress, redacted Run command and Edit actions, input waits, failures, and completion, and attaches the draft pull request URL to the session. Product and Architecture may each use one structured human-input round; the journal is uploaded, the disposable sandbox exits, the parent moves to Needs Input, and the complete question is shown both in the AgentSession chat and in a question-thread comment. A human response in the AgentSession chat, a reply to that exact issue thread, or an Inbox answer records the same canonical response, returns the issue to In Progress, and queues a fresh sandbox that restores the journal. All other stages are forbidden from entering a human-input wait state. Child issues are created directly in Todo, move to In Progress when a coder claims them, move to Done after their commit is integrated, and move to Needs Input on failure. A paused parent also moves to Needs Input. A completed pipeline moves the parent to For Review; the signed GitHub merge webhook moves it to Done. Workflow IDs are discovered from the configured team rather than hard-coded.
